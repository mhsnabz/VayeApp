...
public class MajorPostService {
    ...
    public void setLike(CurrentUser currentUser , LessonPostModel post , TrueFalse<Boolean> result ){
        ...
        if (!post.getLikes().contains(currentUser.getUid())){
            ...
            PushNotificationService.shared().sendPushNotification("empty",currentUser,PushNotificationType.like,String.valueOf(Calendar.getInstance().getTimeInMillis()),post.getSenderUid(),null,PushNotificationTarget.like,currentUser.getName(),post.getText(),MajorPostNotification.descp.post_like,currentUser.getUid());
        }
        else{
